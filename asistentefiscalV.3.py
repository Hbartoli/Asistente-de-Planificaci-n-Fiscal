...
# Definición global de las tareas para la hoja de ruta
LISTA_TAREAS_GLOBAL = [
...
# Función para generar el PDF con márgenes corregidos y tabulación limpia hacia la derecha
...
# -------------------------------------------------------------------------
# PESTAÑA 2: SIMULADOR FINANCIERO MENSUAL INTERACTIVO
# -------------------------------------------------------------------------
with tab2:
    ...
    with col_in_2:
        ...
        alicuota_iibb = st.slider("Alícuota estimada de Ingresos Brutos (%):", min_value=0.0, max_value=5.0, value=3.5, step=0.5, key="mensual_iibb")
...
with tab3:
    ...
    tipo_calculo = st.radio(
        ...
        ("A partir del valor Neto (Quiero que me queden $X de bolsillo)", 
         "A partir del valor Total (Le pasé un presupuesto final de $X al cliente)")
    )
    
    col_f1, col_f2 = st.columns(2)
    with col_f1:
        monto_ingresado = st.number_input("Ingresá el monto de referencia ($):", min_value=0, value=100000, step=10000)
        iva_seleccionado = st.selectbox("Alícuota de IVA aplicable:", [21.0, 10.5, 0.0], index=0)
    with col_f2:
    ...
    st.markdown("---")
    st.subheader("📝 Desglose de la Factura a Emitir:")cf1, cf2, cf3 = 
    st.columns(3)
    with cf1:
        st.metric(label="Subtotal (Neto Gravado)", 
                  value=f"${neto_factura:,.2f}")with cf2:
                      st.metric(label="IVA ({:.1f}%)".format(iva_seleccionado), 
                                value=f"${iva_factura:,.2f}")with cf3:
                                    st.metric(label="Total de la Factura", 
                                              value=f"${total_factura:,.2f}")
